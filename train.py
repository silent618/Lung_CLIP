...
if __name__ == "__main__":
    ...
    for epoch in range(hparams.Classifier.epochs):
        for img_data1, img_data2, table_data in data_loader:
            img_data1.cuda()
            img_data2.cuda()
            table_data.cuda()
            ...
            t_stage, n_stage, m_stage, cancer_type = downstream_model(img_features, table_features)
            loss1 = criterion(t_stage, t_stage_labels)
            loss2 = criterion(n_stage, n_stage_labels)
            loss3 = criterion(m_stage, m_stage_labels)
            loss4 = criterion(cancer_type, cancer_type_labels)
